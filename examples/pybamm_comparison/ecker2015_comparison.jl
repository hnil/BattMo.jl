# # Ecker 2015 Parameter Set – BattMo.jl vs PyBaMM Comparison
#
# This example compares discharge curves between BattMo.jl (Chen 2020) and
# PyBaMM (Ecker 2015) at multiple C-rates.
#
# The Ecker 2015 parameter set is available in PyBaMM's standard library but
...
ax = Axis(fig[1, 1];
	title  = "Ecker 2015 (PyBaMM) vs Chen 2020 (BattMo.jl) – CC Discharge",
	xlabel = "Time [s]",
	ylabel = "Voltage [V]",
)
...
for (i, Crate) in enumerate(c_rates)
	...
	lines!(ax, result_pybamm.time, result_pybamm.voltage;
		...
		label = "PyBaMM Ecker2015 $(Crate)C")
end
