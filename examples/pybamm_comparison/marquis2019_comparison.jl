# # Marquis 2019 Parameter Set – BattMo.jl vs PyBaMM Comparison
#
# This example compares discharge curves between BattMo.jl (Chen 2020) and
# PyBaMM (Marquis 2019) at multiple C-rates.
#
# The Marquis 2019 parameter set is available in PyBaMM's standard library but
...
ax = Axis(fig[1, 1];
	title  = "Marquis 2019 (PyBaMM) vs Chen 2020 (BattMo.jl) – CC Discharge",
	xlabel = "Time [s]",
	ylabel = "Voltage [V]",
)
...
for (i, Crate) in enumerate(c_rates)
	...
	lines!(ax, result_pybamm.time, result_pybamm.voltage;
		...
		label = "PyBaMM Marquis2019 $(Crate)C")
end
